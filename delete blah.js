...
//   if (isNaN(eventIndex)) { // Added the missing closing parenthesis here
//     alert('Please enter a valid event number.');
//     return;
//   }
...
// Event listener to trigger the delete event when the user confirms
document.getElementById('confirmDelete').addEventListener('click', deleteEvent);

// Event listener to cancel the delete operation and close the modal
document.getElementById('cancelDelete').addEventListener('click', closeDeletePrompt);

// This function loads the event details and adds event listeners for delete actions
...
//   if (eventIndex !== null && !isNaN(eventIndex) && eventIndex !== "") {
//     const indexToDelete = parseInt(eventIndex) - 1; // Subtracting 1 because arrays are 0-indexed
    
//     // Make sure 'events' is properly structured and 'clicked' is the correct date or unique identifier
//     events = events.filter((e, index) => {
//       // Ensure the event is deleted either by date or by index
//       return e.date !== clicked || index !== indexToDelete;
//     });

//     // Store the updated events back into localStorage
//     localStorage.setItem('events', JSON.stringify(events));

//     // Close modals and reset input field
//     closeModal();
//     closeDeletePrompt();
//     eventIndexInput.value = '';
//   } else {
